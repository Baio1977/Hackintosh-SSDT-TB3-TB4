DefinitionBlock ("", "SSDT", 2, "INTEL ", "TcssSsdt", 0x00001000)
{
    ...
    Scope (\_SB.PCI0)
    {
        ...
        Method (IMMC, 4, Serialized)
        {
            ...
            Switch (ToInteger (Arg0))
            {
                ...
                Case (0x03)
                {
                    ...
                    {
                        Debug = "IMMC: Get HPD Count - Command Failed"
                        ADBG ("Timeout!")
                ...
                Case (0x06)
                {
                    ...
                    If (((IMCD & 0x80000000) != Zero))
                    {
                        Debug = "IMMC: Decrement HPD Count - Command Failed"
                        ADBG ("Timeout!")
                        CMST = One
                    }
                    ElseIf (((IMCD & 0xFF) != Zero))
        ...
        Device (TXHC)
        {
            ...
            Name (DCPM, 0x04)
            Method (TCLD, 1, Serialized)
            {
                Return (\_SB.PCI0.TDM0.TCLE (Arg0, 0x08))
            }
            ...
            If (OSDR)
            {
                Method (_PS0, 0, Serialized)  // _PS0: Power State 0
                {
                    ...
                    If (OSDW ())
                    {
                        Debug = "_PS0 CPU XHCI TC COLD EXIT (0)"
                        \_SB.PCI0.TDM0.TCLE (Zero, 0x08)
                    }
                    ElseIf ((\_SB.PCI0.TXHC.PMEE == One))
                    {
                        ADBG ("Clear PME_EN of CPU XHCI")
                        Debug = "_PS3 CPU XHCI CLEAR PME"
                ...
                Method (_PS3, 0, Serialized)  // _PS3: Power State 3
                {
                    ...
                    If (OSDW ())
                    {
                        Debug = "_PS3 CPU XHCI GO TO D3"
                        \_SB.PCI0.TXHC.PMEE = One
                        Stall (0x1E)
                        \_SB.PCI0.TXHC.D0D3 = 0x03
                        Stall (0x1E)
                        Debug = "_PS3 CPU XHCI TC COLD ENTER (1)"
                        \_SB.PCI0.TDM0.TCLE (One, 0x08)
                    }
                    ElseIf ((\_SB.PCI0.TXHC.PMEE == Zero))
                    {
                        ADBG ("Set PME_EN of CPU XHCI")
                        Debug = "_PS3 CPU XHCI ENABLE PME"
                        \_SB.PCI0.TXHC.PMEE = One
                    }
        ...
        Device (TDM0)
        {
            ...
            Name (CLDS, Zero)
            Name (DCNT, Zero)
            Method (TCLE, 2, Serialized)
            {
                If ((Arg0 == One))
                {
                    DCLD = (Arg1 | DCLD)
                    Debug = Concatenate (ToHexString (DCLD), " TCLE - TC Cold device opted in, mask for enable is")
                }

                If ((Arg0 == Zero))
                {
                    DCLD = (~Arg1 & DCLD)
                    Debug = Concatenate (ToHexString (DCLD), " TCLE - TC Cold device opted out, mask for enable is")
                }
                ...
                If ((DCNT == 0x03))
                {
                    If ((DCLD == 0x7F))
                    {
                        If ((CLDS == Zero))
                        {
                            Debug = Concatenate (ToHexString (DCLD), " TCLE - Entering TCCold")
                            \_SB.PCI0.TCOF ()
                            CLDS = One
                        }
                    }
                    ElseIf ((CLDS == One))
                    {
                        Debug = Concatenate (ToHexString (DCLD), " TCLE - Exiting TCCold")
                        \_SB.PCI0.TCON ()
                        CLDS = Zero
                    }
                }
                ...
                Return (DCLD) /* \_SB_.PCI0.TDM0.DCLD */
            }

            Method (TCLD, 1, Serialized)
            {
                Return (TCLE (Arg0, 0x07))
            }

            Method (DINC, 0, Serialized)
            {
                DCNT = (One | DCNT)
            }
        ...
        Device (TDM1)
        {
            ...
            Method (TCLD, 1, Serialized)
            {
                Return (\_SB.PCI0.TDM0.TCLE (Arg0, 0x70))
            }

            Method (DINC, 0, Serialized)
            {
                \_SB.PCI0.TDM0.DCNT = (0x02 | \_SB.PCI0.TDM0.DCNT)
            }
    ...
    If (CondRefOf (\_SB.PCI1))
    {
        Scope (\_SB.PCI1)
        {
            Device (TRP0)
            {
                ...
                Name (STAT, One)
                Method (RTST, 1, Serialized)
                {
                    Debug = Concatenate (ToHexString (Arg0), " RTST - Should be 1 if called")
                    STAT = Arg0
                }
